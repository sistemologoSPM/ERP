inherited frmClientesTerminosPagos: TfrmClientesTerminosPagos
  BorderStyle = bsDialog
  Caption = 'Terminos de pagos'
  ClientHeight = 521
  ClientWidth = 729
  ExplicitWidth = 735
  ExplicitHeight = 550
  PixelsPerInch = 96
  TextHeight = 16
  inherited StatusBar1: TStatusBar
    Top = 502
    Width = 729
    ExplicitTop = 492
    ExplicitWidth = 804
  end
  inherited Panel3: TPanel
    Width = 714
    Height = 492
    ExplicitWidth = 789
    ExplicitHeight = 482
  end
  inherited dxLayoutControl1: TdxLayoutControl
    Width = 729
    Height = 502
    ExplicitWidth = 804
    ExplicitHeight = 492
    inherited btnNuevo: TcxButton
      OnClick = btnNuevoClick
    end
    inherited GridListado: TcxGrid
      Width = 470
      Height = 366
      ExplicitWidth = 470
      ExplicitHeight = 366
      inherited GridListadoDBTableView1: TcxGridDBTableView
        DataController.DataSource = dsTerminosDePagos
        object GridListadoDBTableView1TeminoPago: TcxGridDBColumn
          DataBinding.FieldName = 'TerminoPago'
          Width = 200
        end
        object GridListadoDBTableView1Dias: TcxGridDBColumn
          Caption = 'D'#237'as'
          DataBinding.FieldName = 'Dias'
          HeaderAlignmentHorz = taCenter
          Width = 75
        end
        object GridListadoDBTableView1Estado: TcxGridDBColumn
          DataBinding.FieldName = 'Estado'
          PropertiesClassName = 'TcxImageComboBoxProperties'
          Properties.Images = dmBaseDeDatos.ImgenesEstados
          Properties.Items = <
            item
              Description = 'Activo'
              ImageIndex = 0
              Value = 'Activo'
            end
            item
              Description = 'Inactivo'
              ImageIndex = 1
              Value = 'Inactivo'
            end>
          Width = 100
        end
      end
    end
    object btnCambiarEstado: TcxButton [3]
      Left = 520
      Top = 147
      Width = 165
      Height = 30
      OptionsImage.Images = dmBaseDeDatos.ImgenesEstados
      OptionsImage.Margin = 15
      TabOrder = 3
      OnClick = btnCambiarEstadoClick
    end
    object btnModificar: TcxButton [4]
      Left = 520
      Top = 184
      Width = 165
      Height = 30
      Caption = 'Modificar'
      OptionsImage.Glyph.SourceDPI = 96
      OptionsImage.Glyph.Data = {
        
        E9FCD383C486861E772E5431F159E8B80000000049454E44AE426082}
      OptionsImage.Margin = 15
      TabOrder = 4
      OnClick = btnModificarClick
    end
    object btnHistorialCambios: TcxButton [5]
      Left = 520
      Top = 221
      Width = 165
      Height = 30
      Caption = 'Historial de cambios'
      OptionsImage.Glyph.SourceDPI = 96
      OptionsImage.Glyph.Data = {
        
        4F84FFEFF8036E6DB72C00108D310000000049454E44AE426082}
      OptionsImage.Margin = 15
      TabOrder = 5
      OnClick = btnHistorialCambiosClick
    end
    inherited dxLayoutItem5: TdxLayoutItem
      AlignHorz = ahCenter
      CaptionOptions.Visible = False
      Control = btnCambiarEstado
      ControlOptions.OriginalHeight = 30
      ControlOptions.OriginalWidth = 165
      ControlOptions.ShowBorder = False
    end
    inherited dxLayoutItem7: TdxLayoutItem
      Visible = False
    end
    inherited Acciones: TdxLayoutGroup
      ItemIndex = 4
    end
    object dxLayoutItem3: TdxLayoutItem
      Parent = Acciones
      AlignHorz = ahCenter
      CaptionOptions.Text = 'New Item'
      CaptionOptions.Visible = False
      Control = btnModificar
      ControlOptions.OriginalHeight = 30
      ControlOptions.OriginalWidth = 165
      ControlOptions.ShowBorder = False
      Index = 3
    end
    object dxLayoutItem4: TdxLayoutItem
      Parent = Acciones
      AlignHorz = ahCenter
      CaptionOptions.Text = 'New Item'
      CaptionOptions.Visible = False
      Control = btnHistorialCambios
      ControlOptions.OriginalHeight = 30
      ControlOptions.OriginalWidth = 165
      ControlOptions.ShowBorder = False
      Index = 4
    end
  end
  object qTerminosDePagos: TADOQuery
    Connection = dmBaseDeDatos.db
    CursorType = ctStatic
    AfterScroll = qTerminosDePagosAfterScroll
    Parameters = <>
    SQL.Strings = (
      'Select a.* '
      ', b.Nombre Estado'
      
        ', (select count(*) From HistorialDeCambios where tabla = '#39'Client' +
        'esTerminosDePagos'#39' '
      'and ClavePrimaria = a.TerminoPagoId) cambios'
      'From ClientesTerminosdePagos a'
      'Inner join Estados b on a.EstadoId = b.EstadoId')
    Left = 72
    Top = 248
    object qTerminosDePagosTerminoPagoId: TIntegerField
      FieldName = 'TerminoPagoId'
    end
    object qTerminosDePagosDias: TIntegerField
      Alignment = taCenter
      FieldName = 'Dias'
    end
    object qTerminosDePagosEstadoId: TIntegerField
      FieldName = 'EstadoId'
    end
    object qTerminosDePagosCreadoPor: TStringField
      FieldName = 'CreadoPor'
      Size = 50
    end
    object qTerminosDePagosFechaCreacion: TDateTimeField
      FieldName = 'FechaCreacion'
    end
    object qTerminosDePagosModificadoPor: TStringField
      FieldName = 'ModificadoPor'
      Size = 50
    end
    object qTerminosDePagosFechaModificacion: TDateTimeField
      FieldName = 'FechaModificacion'
    end
    object qTerminosDePagosEstado: TStringField
      FieldName = 'Estado'
      Size = 50
    end
    object qTerminosDePagoscambios: TIntegerField
      FieldName = 'cambios'
      ReadOnly = True
    end
    object qTerminosDePagosTerminoPago: TStringField
      FieldName = 'TerminoPago'
      Size = 150
    end
  end
  object dsTerminosDePagos: TDataSource
    DataSet = qTerminosDePagos
    Left = 184
    Top = 256
  end
  object ClientesTerminosDePagosActualizarEstado: TADOStoredProc
    Connection = dmBaseDeDatos.db
    ProcedureName = 'ClientesTerminosDePagosActualizarEstado;1'
    Parameters = <
      item
        Name = '@RETURN_VALUE'
        DataType = ftInteger
        Direction = pdReturnValue
        Precision = 10
      end
      item
        Name = '@TerminoPagoId'
        Attributes = [paNullable]
        DataType = ftInteger
        Precision = 10
      end
      item
        Name = '@EstadoId'
        Attributes = [paNullable]
        DataType = ftInteger
        Precision = 10
      end
      item
        Name = '@usuario'
        Attributes = [paNullable]
        DataType = ftString
        Size = 50
      end>
    Left = 384
    Top = 248
  end
end
